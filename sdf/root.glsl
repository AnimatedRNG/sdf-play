...
float columnridges(in vec3 p, in float radius, in float height,
    ...
    for (int i = 0; i < numridges; i++) {
        ...
        float ridge = fCapsule(p + vec3(radius, 0, 0), ridgeradius,
...
float column(in vec3 p, in float radius, in float height,
    ...
    float base1 = fCylinder(p - vec3(0, height, 0), radius + baseoffset,
                            baseheight);
    float basebox1 = fBox(p - vec3(0, height + 2 * baseheight, 0),
                          vec3(radius + baseoffset + 0.1, baseheight, radius + baseoffset + 0.1));
    base1 = fOpUnionColumns(base1, basebox1, 0.1, 3);
    float base2 = fCylinder(p + vec3(0, height, 0), radius + baseoffset,
                            baseheight);
    float basebox2 = fBox(p + vec3(0, height + 2 * baseheight, 0),
                          vec3(radius + baseoffset + 0.1, baseheight, radius + baseoffset + 0.1));
...
float window(in vec3 p, in float wall, in float width, in float height,
             in float depth,
             in bool arc, in int widthdivisions, in int heightdivisions,
             in float dividerthickness) {
    ...
    float widthbars = fBox(prepeat, vec3(width / widthdivisions, dividerthickness,
                                         dividerthickness));
    float heightbars = fBox(prepeat, vec3(dividerthickness,
                                          height / heightdivisions, dividerthickness));
...
float subtractblock(in vec3 p, in float width, in float height, in float length,
                    float windowwidth, float windowheight, float windowdepth) {
    ...
    for (int i = 0; i < 4; i++) {
        pR(p.xz, i * 3.1414 / 2);
        vec3 prepeat = p + vec3(0, +float(arc) * windowwidth / 2,
                                +length - windowdepth);

        pMod2(prepeat.xy, vec2(windowwidth * 3,
                               (windowheight + windowwidth * float(arc)) * 2));
        windows = min(windows, window(prepeat, block, windowwidth, windowheight,
                                      windowdepth,
                                      arc, widthdivisions, heightdivisions, dividerthickness));

    }
    //return max(windows, block);
    float subblock = fBox(p, vec3(width - windowdepth, height,
                                  length - windowdepth));
    return min(max(windows, block), subblock);
}

float subtractcolumnblock(in vec3 p, in float width, in float height,
                          in float length) {
    /*float columns = 1e10;
    for (int i = 0; i < 4; i++) {
        pR(p.xz, 3.1414 / 2);
    ...
    vec3 pc = p;
    pMod2(pc.xz, vec2(4.0, 4.0));
    float columns = column(pc, 1, height, 0.2, 0.3, 0.05, 20);
    return max(fBox(p, vec3(width, height, length)), -columns);
}
...
float sdf(in vec3 p) {
    ...
    float columnobj = column(p, radius, height, baseoffset, baseheight, ridgeradius,
                             numridges);
    ...
    float windowobj = window(p, wall, windowwidth, windowheight, windowdepth, arc,
                             widthdivisions, heightdivisions, dividerthickness);
    //return windowobj;
    //return wall;
    ...
    vec3 dims = vec3(30, 13, 30);
    vec2 margins = vec2(abs(cos(time)) * 5.0 + 2.0);

    float building = fBox(p, dims);
    building = max(building, -subtractcolumnblock(p +
                   vec3(-dims.x + margins.x - 0.1, 12.0, 0.0),
                   margins.x, 4.2, 31 - margins.y * 2));
    building = max(building, -subtractcolumnblock(p +
                   vec3(dims.x - margins.x + 0.1, 12.0, 0.0),
                   margins.x, 4.2, 31 - margins.y * 2));
    building = max(building, -subtractcolumnblock(p +
                   vec3(0.0, 12.0, -dims.z + margins.y - 0.1),
                   31 - margins.x * 2, 4.2, margins.y));
    building = max(building, -subtractcolumnblock(p +
                   vec3(0.0, 12.0, dims.z - margins.y + 0.1),
                   31 - margins.x * 2, 4.2, margins.y));

    building = max(building, -subtractblock(p + vec3(0.0, 3.0 - 2.0 * abs(cos(time)),
                                                     dims.z - margins.y + 0.1),
                                            31 - margins.x * 2, 4.2, margins.y,
                                            2, 2, 0.5));

    building = max(building, -subtractblock(p + vec3(0.0, -5.0 - 2.0 * abs(cos(time)),
                                                     dims.z - margins.y + 0.1),
                                            31 - margins.x * 2, 4.2, margins.y,
                                            2, 2, 0.5));
